...
public class FriendshipsAPI extends WeiboAPI {
	 ...
	 * @param trim_status
	 *            返回值中user字段中的status字段开关，false：返回完整status字段、true：status字段仅返回status_id，默认为true。
	 * @param listener
	 */
	...
	public void inCommon(long uid, long suid, int count, int page, boolean trim_status, RequestListener listener) {
		...
		request(SERVER_URL_PRIX + "/friends/in_common.json", params, HTTPMETHOD_GET, listener);
	}
	...
	public void bilateral(long uid, int count, int page, RequestListener listener) {
		...
		params.add("count", count);
		params.add("page", page);
		request(SERVER_URL_PRIX + "/friends/bilateral.json", params, HTTPMETHOD_GET, listener);
	}
	...
	public void bilateralIds(long uid, int count, int page, RequestListener listener) {
		...
		params.add("count", count);
		params.add("page", page);
		request(SERVER_URL_PRIX + "/friends/bilateral/ids.json", params, HTTPMETHOD_GET, listener);
	}
	 ...
	 *            返回结果的游标，下一页用返回值里的next_cursor，上一页用previous_cursor，默认为0。
	 * @param trim_status
	 *            返回值中user字段中的status字段开关，false：返回完整status字段、true：status字段仅返回status_id，默认为false。
	 * @param listener
	 */
	...
	public void chainFollowers(long uid, int count, int page, RequestListener listener) {
		...
		params.add("count", count);
		params.add("page", page);
		request(SERVER_URL_PRIX + "/friends_chain/followers.json", params, HTTPMETHOD_GET, listener);
	}
