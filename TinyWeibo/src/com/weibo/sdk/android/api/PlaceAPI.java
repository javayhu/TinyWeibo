...
public class PlaceAPI extends WeiboAPI {
	 ...
	 * @param listener
	 */
	public void nearbyTimeline(String lat, String lon, int range, long starttime, long endtime, SORT3 sort, int count,
			int page, boolean base_app, boolean offset, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.add("lat", lat);
		params.add("long", lon);
	 ...
	 *            返回结果的页码，默认为1。
	 * @param sort
	 *            排序方式，0：按时间、1：按热门，默认为0，目前只支持按时间。
	 * @param base_app
	 *            是否只获取当前应用的数据。false为否（所有数据），true为是（仅当前应用），默认为false。
	 * @param listener
	 */
	public void poisPhotos(String poiid, int count, int page, SORT2 sort, boolean base_app, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
	 ...
	 * @param listener
	 */
	public void poisTips(String poiid, int count, int page, SORT2 sort, boolean base_app, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.add("poiid", poiid);
		params.add("count", count);
		params.add("page", page);
	...
	public void nearbyPois(String lat, String lon, int range, String q, String category, int count, int page,
			boolean offset, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.add("lat", lat);
		params.add("long", lon);
		params.add("range", range);
	 ...
	 * @param listener
	 */
	public void nearbyUsers(String lat, String lon, int range, long starttime, long endtime, SORT3 sort, int count,
	 ...
	 * @param listener
	 */
	public void nearbyPhotos(String lat, String lon, int range, long starttime, long endtime, SORT3 sort, int count,
			int page, boolean offset, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.add("lat", lat);
		params.add("long", lon);
	 ...
	 * @param pic
	 *            需要上传的图片路径，仅支持JPEG、GIF、PNG格式，图片大小小于5M。例如：/sdcard/pic.jgp；注意：pic不能为网络图片
	 * @param isPublic
	 *            是否同步到微博，默认为不同步。
	 * @param listener
	 */
	public void poisAddCheckin(String poiid, String status, String pic, boolean isPublic, RequestListener listener) {
		...
		request(SERVER_URL_PRIX + "/pois/add_checkin.json", params, HTTPMETHOD_POST, listener);
	}
	 ...
	 *            签到时发布的动态内容，内容不超过140个汉字。
	 * @param pic
	 *            需要上传的图片，仅支持JPEG、GIF、PNG格式，图片大小小于5M。例如：/sdcard/pic.jgp；注意：pic不能为网络图片
	 * @param isPublic
	 *            是否同步到微博，默认为不同步。
	 * @param listener
	 */
