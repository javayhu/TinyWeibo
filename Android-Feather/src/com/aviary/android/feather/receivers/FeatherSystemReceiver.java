...
/**
 * Main receiver used to listen for new packages installations. When a package, which is recognized as a feather plugin,
 * has been installed/removed or updated this receiver will broadcast a notification.
 * 
 * @author alessandro
 * 
 */
public class FeatherSystemReceiver extends BroadcastReceiver {
	...
	/**
	 * A package in the system has been installed/replaced or removed. Check if the package is a valid feather plugin
	 * and send a broadcast notification of the newly package. A valid feather plugin must have a resource with the
	 * follwing integer identifiers: is_sticker indicates it's a sticker pack plugin is_filter indicates it's a filter
	 * pack plugin is_tool indicates it's a new tool plugin ( not used )
	 * 
	 * 这个receiver是用来监听系统中的一个包被安装了,被更新了或者被删除了
	 * 检查这个包是否是一个合格的feather plugin,如果是的话发送一个广播通知
	 * 一个合格的feather plugin必须有一个整数标示符
	 * is_sticker 表示它是一个sticker pack插件 
	 * is_filter 表示它是一个filter pack插件
	 * is_tool 表示它是一个tool pack插件
	 * 
	 *  aviary是采用插件机制的,部分工具是免费的,其他的可以通过插件的形式安装,但是要收费
	 *  
	 * @param context
	 *            the context
	 * @param intent
	...
	private void handlePackageAdded(Context context, Intent intent) {
		...
		Uri data = intent.getData();
		String path = data.getSchemeSpecificPart();// path的后面部分，例如： "//www.google.com/search?q=android"

		Bundle extras = intent.getExtras();
		boolean is_replacing = isReplacing(extras);//从bundle中查看是否是更新
		...
		if (null != path && !is_replacing) {
			if (path.startsWith(FeatherIntent.PLUGIN_BASE_PACKAGE)) {
				...
				context.sendBroadcast(newIntent);//Broadcast the given intent to all interested BroadcastReceivers. 
			}
		}
	}
